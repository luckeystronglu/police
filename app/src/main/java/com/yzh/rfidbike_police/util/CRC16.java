...
public class CRC16 {
	...
	static final int[] CRCTable = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084,
	...
	public static byte[] calcCRC(byte[] data) {
		return calcCRC(data, 0, data.length-1);
	}
	
	public static byte[] calcCRC(List<Byte> data, int startIndex, int toIndex) {
		short crc = 0x0000;		
	...
	public static byte[] calcCRC(byte[] data, int startIndex, int toIndex) {		
		short crc = 0x0000;		
		for (int i = startIndex; i <= toIndex; i++) {
			byte b = data[i];			
			crc = (short) ((crc << 8) ^ CRCTable[((crc >> 8 ^ b) & 0xFF)]);
	 ...
	 * @return
	 */
	public static boolean isValidCRC(byte[] bytes) {
		byte[] validCRC = CRC16.calcCRC(bytes, 0, bytes.length - 2 - 1);
		int crcStartIndex = bytes.length - 2;
		if (validCRC[1] == bytes[crcStartIndex]
				&& validCRC[0] == bytes[crcStartIndex + 1]) {
			return true;
		}
		return false;
	}
