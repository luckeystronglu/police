 ...
 */
public class SocketDeviceClient {

	public SocketDeviceClient() {

	}

	private static SocketDeviceClient single = null;

	// 静态工厂方法
	public static SocketDeviceClient getInstance() {
		if (single == null) {
			single = new SocketDeviceClient();
		}
		return single;
	}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				ConnectFuture connect = connector
						.connect(new InetSocketAddress(App.DEVICE_SERVER_IP,
								App.DEVICE_SERVER_PORT));// 建立连接
